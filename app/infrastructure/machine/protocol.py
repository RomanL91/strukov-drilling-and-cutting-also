...
BROWSER_ESCAPES = {"{": "%7B", "}": "%7D", '"': "%22", " ": "%20", "^": "%5E"}


def browser_url(url: str) -> str:
    """Готовит адрес пакета к открытию в браузере.

    Средство запуска браузера разбирает адрес по правилам URI и о фигурные
    скобки с кавычками спотыкается — ничего не открывается. Браузер при ручной
    вставке кодирует эти символы сам; здесь делается ровно то же самое.

    Args:
        url: Адрес пакета в том виде, в каком он уходит на станок.

    Returns:
        Адрес, пригодный для передачи браузеру.
    """
    return "".join(BROWSER_ESCAPES.get(char, char) for char in url)
